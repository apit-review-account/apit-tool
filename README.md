...
During traffic generation for testing, APIT calculates the interrelation of parameters within an API. This allows for the mutation of multiple parameter values in a single test (one request and response) while ensuring these parameters are not logically interconnected. In practical applications, this method effectively reduces the number of tests by up to 70%, significantly enhancing testing efficiency.

# Quick Start

APIT uses proxy mode for passive scanning, taking the Windows system as an example:

`.\apit-windows-amd64.exe poc -l 127.0.0.1:8888 -f poc.yaml -o vuln.html`

# Built-in Test Vectors

APIT includes hundreds of pre-built test vectors for vulnerability testing stored in `/poc`. These vectors are inserted into mutated tree nodes within APIT's test message generation environment for vulnerability validation. These vectors are designed to cover a wide range of potential security flaws, providing a comprehensive testing approach to identify and address vulnerabilities effectively.
...
```
After loading POC file the APIT will mutate the origin API HTTP request to the following data:
```
POST /post HTTP/1.1
Accept: */*
Accept-Encoding: gzip, deflate, br
